...
public class SeatInventoryRepositoryTest extends AbstractRepository {
    ...
    private void setupTestData() {
        Airline airline = Airline.builder().code("LT").name("LATAM").build();
        entityManager.persistAndFlush(airline);

        Airport origin = Airport.builder().code("OYH").name("Olaya Herrera").city("bucaramanga").build();
        entityManager.persistAndFlush(origin);

        Airport destination = Airport.builder().code("LFS").name("Las Flores").city("Cali").build();
        entityManager.persistAndFlush(destination);

        flight1 = Flight.builder().number("kA196").departureTime(OffsetDateTime.now().plusDays(1)).arrivalTime(OffsetDateTime.now().plusDays(1)
    ...
    @Test @DisplayName("Seat Inventory: Find by Flight ID and cabin")
